...
void CSScan(uint3 DTid, uint GI, uint x, uint f)
{
	...
	uint temp;
	uint l = resultBuffer.Load(511 * 4);
	resultBuffer.InterlockedExchange((DTid.x + 512) * 4, l + bucket[GI].y + x, temp);
}

// record and scan the sum of each bucket
[numthreads(groupthreads, 1, 1)]
void csScanBucketResult2(uint3 DTid : SV_DispatchThreadID, uint3 GTid : SV_GroupThreadID, uint GI : SV_GroupIndex)
{
	uint x1 = offsetBuffer.Load((DTid.x*groupthreads+(512*512) - 1) * 4);   // Change the type of x here if scan other types
	CSScan(DTid, GI, x1, 2);
}
