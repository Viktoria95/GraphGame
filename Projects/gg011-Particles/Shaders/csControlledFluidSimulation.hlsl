...
void csControlledFluidSimulation (uint3 DTid : SV_GroupID)
{
	...
	{
		particles[tid].position.y = boundaryBottom + epsilon;
		particles[tid].velocity.y = 0.0;
	}

	if (particles[tid].position.y > boundaryTop)
	{
		particles[tid].position.y = boundaryTop - epsilon;
		particles[tid].velocity.y = 0.0;
	}

	if (particles[tid].position.z > boundarySide)
	{
		particles[tid].position.z = boundarySide - epsilon;
	...
	{
		particles[tid].position.x = boundarySide - epsilon;
