...
namespace Egg { namespace Scene
{
	...
	GG_SUBCLASS(ManagerApp, Egg::App)
	protected:
		/// Mesh binder
		...
		std::map<std::string, Egg::Mesh::Indexed::P > indexedMeshes;
	...
	public:
		ManagerApp(Microsoft::WRL::ComPtr<ID3D11Device> device): Egg::App(device){currentCamera=0;}
